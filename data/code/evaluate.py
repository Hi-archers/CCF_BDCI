...
def parse_args():
    ...
    parser.add_argument('--data_file', metavar='KCT_test_public.txt', default= './results/ans.txt',help='Input data file.')  # 数据文件，格式同训练集
    parser.add_argument('--pred_file', metavar='pred_answer.csv',default= './results/all_local.csv', help='Model predictions.')  # CSV预测结果文件
...
def judge(standardResultFile, userCommitFile):
    ...
    try:
        ...
        # debug
        label_df = label_df[:5704]
        label_df = label_df[['qid','answer']]
        ...
        # print("chk_df: ",chk_df.shape,chk_df)
        # chk_df.to_csv('./results/debug.csv',index = False,encoding = 'utf-8')
